...
public class Data {
    ...
    public ArrayList<Mail> showInbox(String Gmail) {
        ...
        return null;
    }//returns a particular user's inbox
    ...
    public String sendMail(Mail body, String gmail) {
        ...
        if(!sentBy.equals(gmail)){
            return "Invalid action: sender email doesn't match";
        }
    ...
    }//to delete a particular mail in the users inbox
    ...
    }//for deleting the user from the system



    ////////////////////////////////////////////////////////////
    public ArrayList<DraftMail> showDrafts(String gmail) {
        for (Inbox i : inboxes) {
            if (i.getEmail().equals(gmail)) {

                ArrayList<DraftMail> result = new ArrayList<>();
    ...
    public String editDraft(int id, String from, DraftUpdateRequest update) {
        ...
        return "Draft not found";
    }//to edit the draft mails present in the inbox

    public String sendDraftmail(DraftMail body) {
        Mail m = body.send();
        String s = body.getSendto();
        return sendMail(m, s);
    }//to send these draft mails to the specified receiver
    ...
    }//to schedule the mail to a particular time so that the mail could be sent during that specified time to the receiver
    ...
    }//for checking if the time has reached to send the scheduled mail
    ...
    public String unSendMail(int id, String from) {
        int i = 0;
        for (Inbox inbox : inboxes) {
            for (int idx = 0; idx < inbox.getMails().size(); idx++) {
                Mail m = inbox.getMails().get(idx);
                if (m.getSentby().equals(from) && m.getId() == id) {
                    inbox.getMails().remove(idx);
                    i++;
                    break;
                }
            }
            for (int idx = 0; idx < inbox.getSentMails().size(); idx++) {
                Mail m = inbox.getSentMails().get(idx);
                if (m.getSentby().equals(from) && m.getId() == id) {
                    inbox.getSentMails().remove(idx);
                    i++;
                    break;
                }
            }
    ...
    public ArrayList<Mail> searchMails(String gmail, String keyword) {
        ...
        return result;
    }//searches all the mails according to a word
